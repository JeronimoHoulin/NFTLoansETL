...
class X2Y2V1Parser(BaseParser):
    def __init__(self, abi_file=None):
        self.address = "0xC28F7Ee92Cd6619e8eEC6A70923079fBAFb86196"
        if abi_file:
            self.abi_file = abi_file
        else:
            self.abi_file = "../abis/x2y2_v1.json"
        self.contract_name = "x2y2_v1"
        super().__init__()
    ...
    def parse(self, _txn, loan):
        ...
        collateral_contract = _txn["loanDetail"][7]
        collateral_id = _txn["loanDetail"][2]
        principal = _txn["loanDetail"][0] / decimalz
        max_repayment = _txn["loanDetail"][1] / decimalz
        loan_start = datetime.fromtimestamp(_txn["loanDetail"][6], tz=timezone.utc)
        loan_start_stamp = _txn["loanDetail"][6]
        loan_mat = datetime.fromtimestamp(
            (_txn["loanDetail"][6] + _txn["loanDetail"][4]), tz=timezone.utc
        )
    ...
    def extract(self, from_block, to_block):
        ...
        for loan in self.historical_active_x2y2_v1_loans:
        ...
        df_loans_x2y2_v1 = pd.DataFrame(all_loans_x2y2_v1)

        return df_loans_x2y2_v1
