...
class X2Y2V2Parser(BaseParser):
    def __init__(self, abi_file=None):
        self.address = "0xFa4D5258804D7723eb6A934c11b1bd423bC31623"
        if abi_file:
            self.abi_file = abi_file
        else:
            self.abi_file = "../abis/x2y2_v2.json"
        self.contract_name = "x2y2_v2"
        super().__init__()
    ...
    def extract(self, from_block, to_block):
        ...
        for loan in self.historical_active_x2y2_v2_loans:
        ...
        df_loans_x2y2_v2 = pd.DataFrame(all_loans_x2y2_v2)

        return df_loans_x2y2_v2
